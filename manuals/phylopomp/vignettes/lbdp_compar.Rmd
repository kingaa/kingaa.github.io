...
stopifnot(packageVersion("phylopomp")>="0.0.999.7")
...
```{r lbdp1}
runLBDP(lambda=1.5,mu=0.8,psi=1,time=5,n0=5) -> x
x |> lineages() -> lbdp_dat
```

```{r lbdp1_plot,echo=FALSE}
plot_grid(
  x |> plot(points=TRUE),
  lbdp_dat |> plot(),
  ncol=1,
...
The following designs a slice through parameter space in the $\lambda$ direction.
