...
```{r lbdp1}
true.params <- data.frame(lambda=1.5,mu=0.8,psi=1,chi=0.1,n0=5)
with(
  true.params,
  runLBDP(lambda=lambda,mu=mu,psi=psi,chi=chi,n0=n0,time=5)
) |>
  freeze(seed=1159254136)-> x
```
...
foreach (
  ...
  with(
    p,
    x |>
      lbdp_pomp(lambda=lambda,mu=mu,psi=psi,chi=chi,n0=n0) |>
      pfilter(Np=Np) |>
      logLik()
  ) -> ll2
...
params |>
  ...
  group_by(lambda,mu,psi,chi,time,n0,name) |>
  reframe(
